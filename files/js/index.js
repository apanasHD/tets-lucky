...
// проверка ?callback
    ...
    let loc = params.get("callback");
    if(params.has('callback')){
        alert('Есть параметр callback, со значением: ' + loc );
    }
//опеределение гео яндексом
jQuery(function($) {
   $(document).ready(function() {
        if (YMaps.location.country != '') {
            let youCountry = (YMaps.location.country);
            if (youCountry === "Россия"){
                $('.title-small span').text(lvovich.cityIn(YMaps.location.city), '...');//склонение города рф
                $("input[name*='phone']").mask("8(999) 999-9999");//маска для инпута
            }
            else {
                $('.title-small span').text('Странах СНГ');
                $("input[name*='phone']").mask("+(380) 999-9999");//маска для инпута в снг(укр для примера)
            }
        }
    });
    // code
...
//счётчик товара
var numberEl = document.querySelectorAll('.lastpack');

var elArr = [].slice.call(numberEl);
elArr.forEach(function(el) {
   //проверка на наличие в storage
    if (localStorage.getItem('contlp') !== null) {
        el.innerHTML = localStorage.getItem("contlp")
    }
    else{
        el.innerHTML = 60  
    }
    var timer = setInterval(function() {
        el.innerHTML -= 1;
        localStorage.setItem('contlp', el.innerHTML)
        if (el.innerHTML <= 7) {
            clearInterval(timer);
        }
    }, 10000)
